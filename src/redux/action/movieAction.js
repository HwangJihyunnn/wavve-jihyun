...
function getMovies() {    
    return async(dispatch) => {
        try {
            ...
            const popularMoiveApi = api.get(`/tv/airing_today?api_key=${API_KEY}&language=ko&page=1`)
            const topRatedApi = api.get(`/tv/top_rated?api_key=${API_KEY}&language=ko&page=1`)
            const upcomingApi = api.get(`tv/popular?api_key=${API_KEY}&language=ko&page=1`)

            /* 장르 정보를 요청하는 api */
            const genreApi = api.get(`/genre/tv/list?api_key=${API_KEY}&language=ko`)
...
const getMovieDetail = (id) => async(dispatch)=> {
    try {
     ...
     const getMovieDetailApi = api.get(`/tv/${id}?api_key=${API_KEY}&language=ko-KR`);      
